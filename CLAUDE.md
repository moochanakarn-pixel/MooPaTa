...
- Activity pages: `/dashboard` (list), `/dashboard/activity/[id]` (detail), `/dashboard/log-activity`
  ...
  - **ข้อมูลอ้างอิงท่าเวทจากประวัติ (progressive overload) + PR ต่อท่า** — ทั้งคู่มาจาก query เดียวกัน
    ...
    - **RPE (Rate of Perceived Exertion) — คนละความหมายกันระหว่างเวทกับคาร์ดิโอ เลยแยกเป็น 2 field
      ...
      เท่านั้น ค่านอกช่วงทำให้ request ทั้งก้อนพังเหมือน field optional อื่น ๆ) — **เจอบั๊กจริงจากการใช้งาน:
      ช่อง RPE ทั้งสองจุดในฟอร์ม (`log-activity-form.tsx`) เป็น `<input type="number" min="1" max="10">`
      ไม่มี `step="1"` เลย ทำให้พิมพ์ทศนิยมได้ (เช่น `8.5`) ผ่านหน้าฟอร์มเงียบ ๆ พอกด "บันทึกกิจกรรม" แล้ว
      ไปโดน `optionalRpe()`'s `Number.isInteger` reject ที่ server (400 `invalid_optional_field`) —
      ฝั่ง client จับ error แบบรวม ๆ (`res.ok` เป็น false ก็โชว์แค่ "บันทึกไม่สำเร็จ ลองใหม่อีกครั้ง"
      ไม่บอกว่าฟิลด์ไหนพัง) ทำให้ debug ยากมาก ต้องไล่ทีละฟิลด์เอง — แก้ 2 จุด: (1) เพิ่ม `step="1"` ให้
      ทั้งสอง input กัน browser/มือถือเสนอปุ่มจุดทศนิยม (2) เพิ่มเช็ค client-side ใน `save()` ก่อนยิง
      request (ทั้ง RPE กิจกรรมทั้งครั้งและ RPE รายเซ็ท) ให้ขึ้น error message เจาะจงว่า "ต้องเป็นจำนวนเต็ม
      1-10 (ห้ามมีจุดทศนิยม)" แทนข้อความรวม ๆ เดิม — **ข้อความ error รวม ๆ แบบนี้ยังเป็นปัญหากับ field
