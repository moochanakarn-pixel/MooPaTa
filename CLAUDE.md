...
- **การ์ดแชร์กิจกรรมเดี่ยว** (`/api/share/[id]/route.tsx`, ปุ่ม "แชร์" ที่หน้ารายละเอียดกิจกรรม เปิด
  ...
  - **`?style=list` เคยโหลดค้างบ่อยสำหรับเซสชันเวทจริง ๆ (ไม่ใช่แค่เคสสุดโต่ง)** — ผู้ใช้แจ้งจากหน้าจอ
    ...
    ลดเวลาลง ~27-45% (32 เซ็ท 33→24 วิ, 90 เซ็ท 136→78 วิ) — (2) **`MAX_LIST_SETS = 80`** เพดานจำนวนเซ็ท
    ...
    `Activity.notes`'s 500-ตัวอักษร — เพดานนี้ทำให้ 300-เซ็ทที่เคยค้าง 10+ นาทีไม่จบ กลายเป็น ~68 วิ (ใกล้
    เคียงกับ 90-เซ็ทที่ถูกตัดเหลือ 80 เหมือนกัน พิสูจน์ว่าเพดานทำงานถูกต้อง) — 80 เป็นเลขที่ตั้งใจให้สูงพอ
    จนแทบไม่มีทางโดนตัดจริงสำหรับ session เดี่ยวปกติ (แม้จะหนักขนาด 15 ท่า×5-6 เซ็ท=75-90 ก็ยังใกล้ๆ/ต่ำกว่า
    เพดานนิดเดียว) เจาะจงกันแค่เคสสุดโต่ง/ข้อมูลผิดปกติ (เช่น import ซ้ำ) — ทั้งสองจุดแก้ที่ height-estimation
    ต้องอิง `visibleExercises` (array ที่ตัดตามเพดานแล้ว) ไม่ใช่ `activity.exercises` ดิบ ไม่งั้นความสูง
    canvas จะคำนวณเผื่อเนื้อหาที่ไม่ได้ render จริง (พลาดแบบเดียวกับบั๊ก PR-badge wrap เดิมด้านบน) —
    เพิ่มด้วย: ข้าม query `getExerciseStats` (หา PR badge) ไปเลยตอน `cardStyle === "list"` เพราะสไตล์นี้
    ไม่เคยโชว์ PR badge อยู่แล้ว (ดูด้านบน) เดิมยังคง query อยู่ทุกครั้งที่มี exercises แม้ผลจะไม่ถูกใช้เลย —
    ประหยัด DB roundtrip เปล่า ๆ ไม่ได้กระทบเวลา render (Satori เองคือคอขวด ไม่ใช่ DB) แต่เป็น cleanup ที่ถูก
    ต้องไปด้วย
