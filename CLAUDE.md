...
- **โบนัสแคลอรี่/แมโครจากกิจกรรมวันนี้ ("เพิ่มจากกิจกรรมวันนี้")** — `applyActivityBonus`
  ...
  — เดิมคิดจาก**เวลาออกกำลังกายรวมทั้งวันอย่างเดียว** (`activityDurationSecToday`, ทุก 30 นาทีสะสม
  ทุกประเภทกิจกรรมรวมกัน = +15g คาร์บ +5g โปรตีน, สูงสุด 90g/30g ที่ 3 ชม., ไม่แยกวิ่ง/เวท/ความหนักเลย
  ตั้งใจเป็นแบบนี้เพราะกิจกรรมเก่าจาก Strava sync ไม่มีสัญญาณความหนักที่เทียบข้ามประเภทได้) —
  **ผู้ใช้ถามว่าทำไมกรอกแคลอรี่กิจกรรม (335 → 3350 kcal) แล้วโบนัสไม่ขยับเลย** พบว่าเป็นเพราะสูตรเดิม
  ไม่แตะ `Activity.calories` เลยสักตัวจริง ๆ ไม่ใช่บั๊ก — ผู้ใช้เลยขอให้ผสมเข้าไปด้วยเพราะแคลอรี่มาจาก
  นาฬิกาจริง ไม่ใช่เดาเอา — เพิ่ม **`activityCalorieBonusKcal(loggedCaloriesToday)`** เป็นส่วนเสริม
  ต่อยอดจากสูตรเดิม (ไม่ได้แทนที่): เครดิตคืนแค่ 30% ของแคลอรี่ที่กรอกไว้ในกิจกรรมวันนั้น
  (`CALORIE_BONUS_RETURN_RATE`) เพดานที่ 250 kcal (`MAX_CALORIE_BONUS_KCAL`) — ตั้งใจไม่คืนเต็ม 100%
  เพราะแคลอรี่จากนาฬิกามักประเมินสูงเกินจริง (ปัญหาที่รู้กันทั่วไปของอุปกรณ์ทุกยี่ห้อ) ถ้าคืนเต็มเสี่ยง
  ทำให้คนตั้งเป้าลดน้ำหนักได้โบนัสเกินจริง — **เป็นแค่ส่วนเสริมบวกเพิ่ม ไม่ใช่แทนที่สูตรเวลา** กิจกรรม
  ที่ไม่ได้กรอกแคลอรี่ไว้ (พบบ่อยเพราะ `Activity.calories` เป็น optional) ยังได้โบนัสจากเวลาเท่าเดิม
  ทุกประการ ไม่มีทาง regression ลงเป็น 0 — ค่าที่ได้จาก `activityCalorieBonusKcal` ถูกพับเข้าไปเป็น
  คาร์บเพิ่ม (`carbBonusG += calorieBonusKcal / 4`) ไม่ใช่บวกเข้า `targetCalories` ตรง ๆ เป็นแหล่ง kcal
  ที่ไม่มีที่มา เพื่อให้ `targetCalories` ยังคงเท่ากับผลรวมแมโคร×kcal/g เป๊ะเหมือนเดิม (คาร์บถูกเลือก
  เพราะเป็นแมโครที่ยืดหยุ่นอยู่แล้วตามที่สูตรเวลาเดิมก็ใช้) — `applyActivityBonus` มี argument ตัวที่ 3
  เพิ่มมา (`loggedCaloriesToday`, default `0` กันพังจุดที่ยังไม่ได้ส่ง) — **ทุกจุดที่เรียกต้องส่ง
  ผลรวม `Activity.calories` ของวันนั้นเข้าไปด้วยเสมอ** (เหมือนกติกาเดียวกับ body composition/macro
  prefs ด้านบน) ไม่งั้นตัวเลขจะไม่ตรงกันข้ามหน้า — จุดที่ส่งแล้ว: หน้าแรก (`dashboard/page.tsx`,
  เพิ่ม `calories: true` ใน `_sum`), หน้าเชิงลึก (`nutrition/page.tsx`, มี `activityCaloriesByDay`
  แยกจาก `caloriesByDay` เดิมที่หมายถึงแคลอรี่ที่ "กิน" ไม่ใช่ "เผาผลาญ" — ระวังอย่าสับสนชื่อสองตัวนี้),
  หน้าไดอารี่ (`food/page.tsx`), การ์ดสรุปผลประจำวัน (`daily-summary/route.tsx`) — ส่วน
  `cron/water-reminder` ไม่ต้องส่งเพราะใช้แค่ `.waterMl` ที่ไม่ขึ้นกับ argument นี้เลย — หน้าเชิงลึกโชว์
  breakdown เพิ่ม "(รวมโบนัส +N kcal จากแคลอรี่ที่บันทึกไว้ในกิจกรรมวันนี้ด้วย)" ต่อท้ายบรรทัดเดิมเมื่อ
  `calorieBonusKcal > 0` เท่านั้น (ซ่อนไปเลยถ้าไม่มี ไม่โชว์ "+0 kcal" ที่ไม่มีความหมาย) — หน้าไดอารี่
  (`food-log-view.tsx`) ไม่ต้องแก้อะไรเพิ่ม เพราะ `carbBonusG` ที่ส่งไปแสดงอยู่แล้วรวมส่วนนี้ไว้ในตัวเลข
  เดียวกันโดยอัตโนมัติ — **เวอร์ชันที่ปรับขึ้น/ลงตามจริง (ไม่ใช่แค่บวกเพิ่มทางเดียว) ยังไม่ได้ทำ** เพราะ
  ต้องรู้ประเภท+ความหนักของแต่ละกิจกรรมมาเทียบกับ `estimateCalories` (MET-based) ไม่ใช่แค่ผลรวม
  แคลอรี่/เวลาทั้งวัน ต้องเปลี่ยนทุกจุดข้างบนจาก `aggregate` เป็น `findMany` ดึงข้อมูลราย activity —
  ทิ้งไว้เป็นตัวเลือกถ้าอยากทำต่อในอนาคต
