...
- Activity pages: `/dashboard` (list), `/dashboard/activity/[id]` (detail), `/dashboard/log-activity`
  ...
  - **ข้อมูลอ้างอิงท่าเวทจากประวัติ (progressive overload) + PR ต่อท่า** — ทั้งคู่มาจาก query เดียวกัน
    ...
    - **แต่ละเซ็ทเก็บ reps/น้ำหนักแยกกันได้ (ไม่บังคับเท่ากันทุกเซ็ท)** — เดิม `Exercise` เก็บ
      ...
      - **นำเข้าจาก AI (`activity-import-parse.ts`) ยังไม่รองรับเซ็ทที่ค่าต่างกัน** — ตั้งใจไม่แก้ตอนนี้
        (นอกขอบเขตของรอบนี้) AI ยังตอบกลับมาแบบ `{sets: จำนวน, reps, weightKg}` ค่าเดียวเหมือนเดิม
        ฟอร์มฝั่ง client (`applyParsedText`) เป็นคนขยายให้เป็น N แถวเซ็ทที่ค่าเท่ากันหมดแทน (ผู้ใช้แก้
        ทีละเซ็ทเองต่อได้หลังจากนั้นถ้าจริง ๆ แล้วแต่ละเซ็ทไม่เท่ากัน) — ถ้าจะให้ AI แยกค่าต่อเซ็ทเองในอนาคต
        ต้องแก้ prompt template + parser ทั้งคู่แยกต่างหาก
    - **RPE (Rate of Perceived Exertion) — คนละความหมายกันระหว่างเวทกับคาร์ดิโอ เลยแยกเป็น 2 field
      ...
      - **นำเข้าจาก AI รองรับ RPE ของกิจกรรมทั้งครั้งเท่านั้น ไม่รองรับ RPE รายเซ็ท** — เพิ่ม field
        matcher ใหม่ใน `activity-import-parse.ts` (คีย์เวิร์ด `ระดับความเหนื่อย`/`RPE`) อ่านจากรูปสรุป
        ที่นาฬิกา/สายรัดโชว์ค่าความเหนื่อยไว้แล้ว (ถ้ามี) ใส่ prompt template ด้วย — ตั้งใจไม่แตะ
        `parseExerciseLine`/ตาราง "ชื่อท่า | เซ็ท | ครั้ง | น้ำหนัก" เลย เพราะ RPE รายเซ็ท (เหลือแรงยกได้
        อีกกี่ที) เป็นความรู้สึกส่วนตัวขณะเล่นจริง ไม่ใช่ตัวเลขที่นาฬิกา/สกรีนช็อตจะมีให้อ่านได้ — ต่างจาก
        RPE คาร์ดิโอที่นาฬิกาบางรุ่นคำนวณ/โชว์ให้เองอยู่แล้ว
