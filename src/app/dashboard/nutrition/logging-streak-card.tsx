...
export function LoggingStreakCard({
  ...
  const t = useTranslations("nutrition.loggingStreakCard");
  ...
  return (
    <div className="mb-6 rounded-2xl border border-neutral-800/80 bg-neutral-900/40 p-5">
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 text-sm">
        <div>
          <p className="font-semibold text-neutral-200">{t("daysCount", { count: longestFoodStreak })}</p>
          <p className="text-xs text-neutral-500">{t("longestFoodStreak")}</p>
        </div>
        <div>
          <p className="font-semibold text-neutral-200">{t("daysCount", { count: longestWeightStreak })}</p>
          <p className="text-xs text-neutral-500">{t("longestWeightStreak")}</p>
        </div>
      </div>
      ...
      <div className="flex justify-between border-t border-neutral-800 pt-4">
        {weekDays.map((d, i) => (
          <div key={i} className="flex flex-col items-center gap-1">
            <span className={`text-[10px] ${d.isToday ? "text-lime-400" : "text-neutral-600"}`}>{weekdayLabels[i]}</span>
            <span
