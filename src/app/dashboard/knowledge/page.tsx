  ...
  return (
    <main className="mx-auto max-w-2xl px-6 py-10">
      ...
      <Section title="BMR (พลังงานที่ใช้ตอนนอนนิ่ง ๆ)">
        ...
        <p>
          ถ้ามีผลตรวจ InBody (หรือเครื่องวัดองค์ประกอบร่างกายอื่น) บันทึกไว้ในหน้าเชิงลึก แอปจะสลับไปใช้สูตร <strong>Katch-McArdle</strong>{" "}
          แทน ซึ่งแม่นยำกว่า Mifflin-St Jeor เพราะอิง <strong>มวลไร้ไขมันจริง (Lean Body Mass)</strong> จากผลตรวจ แทนการเดาจาก
          น้ำหนัก/ส่วนสูง/อายุ/เพศเฉลี่ยของประชากร
        </p>
        <Formula>มวลไร้ไขมัน (LBM) = น้ำหนักจากผลตรวจ(กก.) × (1 − %ไขมันจากผลตรวจ ÷ 100)</Formula>
        <Formula>BMR (Katch-McArdle) = 370 + 21.6 × LBM(กก.)</Formula>
        <p>ใช้เฉพาะผลตรวจล่าสุดที่มีค่า %ไขมันระบุไว้เท่านั้น — ถ้ายังไม่เคยกรอกผลตรวจ หรือกรอกแต่ไม่มี %ไขมัน จะใช้สูตร Mifflin-St Jeor ด้านบนแทนตามปกติ</p>
      </Section>
      ...
      <Section title="แมโคร (โปรตีน/คาร์บ/ไขมัน)">
        <ul className="ml-4 list-disc space-y-1">
          <li>โปรตีน (ไม่มีผลตรวจ InBody): 1.8 กรัม ต่อน้ำหนักตัว 1 กก. (อยู่กลางช่วง 1.6-2.2 ก./กก. ที่แนะนำสำหรับคนออกกำลังกายสม่ำเสมอ)</li>
          <li>โปรตีน (มีผลตรวจ InBody ที่ระบุ %ไขมัน): 2.4 กรัม ต่อมวลไร้ไขมัน (LBM) 1 กก. — ปลายบนของช่วง 2.0-2.4 ก./กก. LBM ที่งานวิจัยกีฬาแนะนำ เพราะรู้สัดส่วนไขมัน/กล้ามเนื้อจริงแล้ว ไม่ต้องเผื่อความคลาดเคลื่อนแบบสูตรน้ำหนักตัวเปล่า</li>
          <li>ไขมัน: 25% ของแคลอรี่เป้าหมายทั้งหมด</li>
          <li>คาร์บ: ส่วนที่เหลือหลังหักโปรตีนและไขมันแล้ว</li>
        </ul>
        <p>
          ตัวอย่าง: น้ำหนัก 70.1 กก. %ไขมัน 19.3% → LBM ≈ 56.6 กก. → โปรตีนเป้าหมาย ≈ 56.6 × 2.4 ≈ 136 กรัม/วัน
          (สูงกว่าแบบคำนวณจากน้ำหนักตัวเปล่า เพราะรู้แน่ชัดแล้วว่าน้ำหนักส่วนไหนเป็นกล้ามเนื้อ/อวัยวะที่ต้องการโปรตีนไปเลี้ยงจริง ๆ)
        </p>
      </Section>
